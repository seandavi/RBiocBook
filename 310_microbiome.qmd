...
![Anatomy of a TreeSummarizedExperiment. On top of the assay matrix, `rowData`, and
`colData` it shares with a SummarizedExperiment, it adds slots that record *hierarchy*:
a **`rowTree`** holding a tree over the features (here, the taxonomic/phylogenetic
relationships among microbes) with **`rowLinks`** tying each assay row to a node on
that tree; an optional **`colTree`**/**`colLinks`** doing the same for the samples; and
an optional **`referenceSeq`** storing a reference sequence for each feature. The tree
slots are what make this container a natural fit for microbiome data, where features
are related by a phylogeny.](images/treesummarizedexperiment.png){#fig-tse-anatomy}
