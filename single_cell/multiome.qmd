...
- In **diagonal** integration there is no correspondence at all. You have RNA for one set of cells and ATAC for a different set, and nothing tells you that an RNA cell and an ATAC cell are "the same kind." You must *infer* the alignment, almost always by projecting both into a shared feature space (the bridges below) and matching. This is the case that forces you to understand *why* integration is hard, and we return to it in the [next chapter](atac_rna_label_transfer.qmd) using label transfer.
