...
The heatmap showed marker genes as a block matrix. A complementary view is to paint a single marker gene's expression straight onto a 2-D embedding, so you can see *where* it is expressed. Let's add a **UMAP** — a second non-linear embedding that, like t-SNE, exists purely to make high-dimensional structure viewable (the [concepts chapter](concepts.qmd) explains why these embeddings are views, not measurements).

```{r}
#| label: fig-umap-celltype
#| fig.cap: "A UMAP of the same cells, colored by curated `level1class` cell type. Like the earlier t-SNE, UMAP is a visualization-only projection of the PCA space — and the same cell-type islands reappear, evidence that the structure is real rather than an artifact of one embedding."
library(scater)
library(ggplot2)
set.seed(1234)
...
Reassuringly, the same cell-type islands we saw in the t-SNE reappear: the embedding algorithm changed, but the biology did not.

Now color the map by individual marker genes — the single top-ranked marker for each of several cell types — and watch each gene light up its own territory.
...
4. **Score a different cell type.** Pick another entry of `celltype_names` — say an oligodendrocyte or astrocyte type — recompute the module score for *its* marker set, and redraw the UMAP. Does the high-scoring region land on that cell type's island in @fig-umap-celltype?
