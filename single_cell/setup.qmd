...
library(scrapper)
library(scater)
cat("CIPROBE 1: libraries loaded\n", file = stderr())  # CI diagnostic — remove
```
...
cat("CIPROBE 2: surveyDatasets done\n", file = stderr())  # CI diagnostic — remove
```
...
cat("CIPROBE 3: ZeiselBrainData done\n", file = stderr())  # CI diagnostic — remove
```
...
<!-- DIAG: remote Nature image temporarily removed to test CI multi-format hang -->
:::: {#fig-scRNAseq-sce}
(figure placeholder)
::::
...
cat("CIPROBE 7: clusterGraph done\n", file = stderr())  # CI diagnostic — remove
...
cat("CIPROBE 9: scoreMarkers(celltype) done\n", file = stderr())  # CI diagnostic — remove
```
