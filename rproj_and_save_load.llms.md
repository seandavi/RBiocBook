...
- Save and restore your whole workspace with [`save.image()`](https://rdrr.io/r/base/save.html) and [`load()`](https://rdrr.io/r/base/load.html).
- Save and reload *selected* objects with [`save()`](https://rdrr.io/r/base/save.html) and [`load()`](https://rdrr.io/r/base/load.html).
- Save and replay your command history with [`savehistory()`](https://rdrr.io/r/utils/savehistory.html) and [`loadhistory()`](https://rdrr.io/r/utils/savehistory.html).
...
> **TIP:**
>
> In larger projects — especially ones with notebooks tucked into subfolders — the `here` package makes paths bulletproof. [`here()`](https://here.r-lib.org/reference/here.html) always resolves from the project root (the folder with the `.Rproj` file), no matter which subfolder your script sits in:
>
> ``` downlit
...
The whole collection of objects in your current session is called your **workspace** (or the *global environment*). When you quit R with [`q()`](https://rdrr.io/r/base/quit.html) and answer “yes” to *Save workspace image?*, R writes your entire workspace to a file named `.RData` in the working directory. That is exactly equivalent to running:
...
Saving the *entire* workspace is often more than you want. Usually only a couple of objects are worth keeping — the cleaned dataset, the final result — while the rest is scratch work. The [`save()`](https://rdrr.io/r/base/save.html) function lets you pick exactly which objects to write, and [`load()`](https://rdrr.io/r/base/load.html) brings them back.
...
In a later session, [`load()`](https://rdrr.io/r/base/load.html) drops `months` and `ages` straight back into your workspace — `flags` is not in the file, so it does not come along:
...
> **NOTE:**
>
> [`save()`](https://rdrr.io/r/base/save.html) stores objects *together with their names*, so [`load()`](https://rdrr.io/r/base/load.html) recreates them under those same names. A close cousin, [`saveRDS()`](https://rdrr.io/r/base/readRDS.html), stores a *single* object with no name attached, and you assign it yourself on the way back in: `my_data <- readRDS("file.rds")`. Reach for [`saveRDS()`](https://rdrr.io/r/base/readRDS.html) when you want one object and full control over what it’s called.

## 6.5 Saving and loading your command history

R also remembers the *commands* you type, not just the objects they create. When you save your session on quit, your command history is saved too. You can manage it by hand with [`savehistory()`](https://rdrr.io/r/utils/savehistory.html) and [`loadhistory()`](https://rdrr.io/r/utils/savehistory.html):
...
1.  **Spot the fragile path.** A collaborator sends you a script that begins with `expr <- read.csv("C:/Users/jordan/Desktop/data/counts.csv")`. It runs fine on their laptop but errors on yours. Why, and how would you rewrite the line so it works for both of you?
...
2.  **Save just what matters.** You have three objects in your session — `counts`, `samples`, and a throwaway `tmp`. Write the one line that saves only `counts` and `samples` to a file called `results.RData`, then the line that loads them back in a future session.
    ...
    > ```
    >
    > `tmp` is never named in the [`save()`](https://rdrr.io/r/base/save.html) call, so it isn’t written — and [`load()`](https://rdrr.io/r/base/load.html) restores `counts` and `samples` under their original names.

3.  **One object, your name.** You want to hand a single cleaned data frame, `brfss_clean`, to a colleague who will read it under a name of *their* choosing. Which function fits better, [`save()`](https://rdrr.io/r/base/save.html) or [`saveRDS()`](https://rdrr.io/r/base/readRDS.html), and why?

    > **NOTE:**
    > [`saveRDS()`](https://rdrr.io/r/base/readRDS.html). It stores the object without a name, so your colleague assigns it themselves on the way in:
    >
    > ``` downlit
    ...
    > ```
    >
    > [`save()`](https://rdrr.io/r/base/save.html) would have forced the object back into their workspace as `brfss_clean`, whether they wanted that name or not.
...
- **Checkpoint** a whole session with [`save.image()`](https://rdrr.io/r/base/save.html) / [`load()`](https://rdrr.io/r/base/load.html), preferring named files over the silent `.RData` restore.
- **Save selectively** with [`save()`](https://rdrr.io/r/base/save.html) (objects plus their names) or [`saveRDS()`](https://rdrr.io/r/base/readRDS.html) (one object, you name it on the way back).
- **Keep your history** with [`savehistory()`](https://rdrr.io/r/utils/savehistory.html) / [`loadhistory()`](https://rdrr.io/r/utils/savehistory.html) to turn a messy session into a tidy script.
