...
#| fig-cap: "Element-wise execution lines the two vectors up and operates on each matched pair independently, producing a result of the same length."
#| fig-width: 6.5
#| fig-height: 2.6
#| echo: false
a <- 1:6
...
plot.new()
plot.window(xlim = c(-2, 6.6), ylim = c(0.4, 3.6))
draw_row <- function(vals, y, fill, lab) {
  for (i in seq_along(vals)) {
    rect(i - 0.45, y - 0.42, i + 0.45, y + 0.42, col = fill, border = "grey45")
    text(i, y, vals[i], cex = 1.1)
  }
  text(0.3, y, lab, adj = 1)
}
draw_row(a, 3, "#cfe8ff", "die")
draw_row(a, 2, "#cfe8ff", "die")
draw_row(a * a, 1, "#d6f0d6", "die * die")
```
...
```{r}
#| label: fig-recycle
#| fig-cap: "Vector recycling: the shorter vector (here 1:2) is repeated until it matches the longer one, then the operation runs element-wise. The repeated copies are shown in a lighter shade."
#| fig-width: 6.5
#| fig-height: 2.6
#| echo: false
a <- 1:6
b <- rep(1:2, 3)
fill_b <- c("#ffd08a", "#ffd08a", "#ffe9c6", "#ffe9c6", "#ffe9c6", "#ffe9c6")
op <- par(mar = c(0, 0, 0, 0)); on.exit(par(op))
plot.new()
plot.window(xlim = c(-3, 6.6), ylim = c(0.4, 3.6))
draw_row <- function(vals, y, fill, lab) {
  for (i in seq_along(vals)) {
    rect(i - 0.45, y - 0.42, i + 0.45, y + 0.42, col = fill[i], border = "grey45")
    text(i, y, vals[i], cex = 1.1)
  }
  text(0.3, y, lab, adj = 1)
}
draw_row(a, 3, rep("#cfe8ff", 6), "die")
draw_row(b, 2, fill_b, "1:2 (recycled)")
draw_row(a + b, 1, rep("#d6f0d6", 6), "die + 1:2")
```
...
The data that you pass into the function is called the function's _argument_. The argument can be raw data, an R object, or even the results of another R function. In this last case, R will work from the innermost function to the outermost.
...
```r
roll <- function(bones) {           # name  <-  function(arguments)
  result <- sample(bones, size = 1) # body: the code that runs on a call
  result                            # the last value is returned
}
```

Every function you write has the same three parts: a **name** (the object you
assign it to with `<-`), its **arguments** (the inputs, listed in the
parentheses), and its **body** (the code between the braces). Calling
`roll(1:6)` runs the body with `bones` set to `1:6`.
:::

## Scripts

Scripts are code that are saved for later reuse or editing. An R script is just a plain text file that you save R code in. You can open an R script in RStudio by going to **File > New File > R script** in the menu bar. RStudio will then open a fresh script above your console pane (the
[RStudio chapter](intro_to_rstudio.qmd) tours the panes).
